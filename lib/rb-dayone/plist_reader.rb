...
class DayOne::PlistReader
  ...
  # @param path [String] the path to the PList; defaults to the standard DayOne preference plist
  def initialize(path = nil)
    ...
    @path = if path
              ...
              first_valid_default_path = default_paths.find { |p| File.exist?(p) }
              if first_valid_default_path.nil?
                raise 'Cannot locate DayOne preference file'
              else
  ...
  def key(key_value)
    ...
    key_element.next_element.content if key_element
  end
