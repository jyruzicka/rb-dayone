...
class DayOne::Location
  ...
  STRING_COMPONENTS = %i[country locality administrative_area place_name].freeze
  REAL_COMPONENTS = %i[latitude longitude].freeze
  ...
  def xml_string(symbol)
    if symbol.is_a? String
      ...
      symbol.to_s.tr('_', ' ').gsub(/\b./, &:upcase)
    end
  end
  ...
  def symbol(xml_string)
    if xml_string.is_a? Symbol
      ...
      xml_string.downcase.tr(' ', '_').intern
    end
  end
end
